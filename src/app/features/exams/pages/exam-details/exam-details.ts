...
@Component({
  ...
  imports: [Breadcrumbs, BackArrow, PageTitle, ExamNavigationButton, AsyncPipe, CommonModule, RouterOutlet],
...
export class ExamDetails implements OnInit {
  ...
  submitExam() {
    this._store.dispatch(examActions.submitExam());
  ...
  ngOnInit(): void {
    ...
    this._store.dispatch(examActions.loadQuestions({
     ...
     this._store.dispatch(examActions.setExamId({ examId }));
    this._store.dispatch(
      examActions.setStartedAt({ startedAt: new Date().toISOString() })
    );
